...
function Columns({columns, widgets, onRemove, layout, rowIndex, editable, onAdd, frame, editableColumnClass, droppableColumnClass, onMove}) {
  const items = columns.map((column, index) => {
    return (
      <Column  key={index} className={column.className} onAdd={onAdd} layout={layout} rowIndex={rowIndex} columnIndex={index} editable={editable} onDrop={handleDrop} onMove={onMove} editableColumnClass={editableColumnClass}
