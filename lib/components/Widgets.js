...
const Widgets = ({widgets, widgetTypes, onRemove, layout, columnIndex, rowIndex, editable, frameComponent, onMove}) => {
    console.log('I was triggered during the constant Widgets: ', widgets);
    console.log('I was triggered during the constant layout: ', layout);
    let createdWidgets = widgets.map((widget, index) => { // eslint-disable-line arrow-body-style
        console.log('CREATED WIDGETS: ', widget, index);
        return (
            <WidgetFrame
                key={index}
